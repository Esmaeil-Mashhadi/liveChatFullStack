...
class NameSpaceSocket {
  ...
  constructor(io) {
    this.#io = io;
    this.initConnection();
    this.watchNameSpaceModelChanges(); // Add watcher for changes in nameSpaceModel
  }
  ...
  getCountOfOnlineUsers = async (endpoint, roomName) => {
    const socketConnections = await this.#io
      .of(`/${endpoint}`)
      .in(roomName)
      .allSockets();
    ...
    this.#io
      .of(`${endpoint}`)
      .in(roomName)
      .emit("onlineUsers", onlineUsers.length);
  };
  ...
  getNewMessage = (socket, roomName, endpoint) => {
    socket.on("clientMessage", async ({ message, email }) => {
      if (message) {
        ...
        const result = await nameSpaceModel.updateOne(
      ...
      const nameSpaces = await nameSpaceModel
        ...
        .populate([
          {
            path: "rooms.messages.sender",
            select: ["email", "sender", "userName"],
          },
        ]);
      nameSpaces.forEach((nameSpace) => {
        ...
        if (room) {
          this.#io
            .of(`${endpoint}`)
            .in(roomName)
            .emit("roomInfo", { messages: room.messages, endpoint, roomName });
        }
      });
    });
  };

  watchNameSpaceModelChanges = () => {
    nameSpaceModel.watch().on("change", async (data) => {
      if (data.operationType === "update") {
        const nameSpaces = await nameSpaceModel
          .find({})
          .sort({ _id: -1 })
          .populate([{ path: "rooms.messages.sender", select: ["email", "sender", "userName"] }]);
        // Update the namespaces list for all connected clients
        this.#io.emit("nameSpacesList", nameSpaces);
      }
    });
  };
}
